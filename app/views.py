...
GOOGLE_CLIENT_ID = '985983038574-uevth6gnpbaa5r1vpigak0o4i5oiki23.apps.googleusercontent.com'
GOOGLE_CLIENT_SECRET = 'iC7DTnvR_ZtLh9EtJggG8Ims'
...
@page.route('/')  # Here is the code for getting access token of user and authorizing it with his google account.
def index_g():
...
@page.route('/logingoogle')
def login_with_google():
    callback=url_for('authorized', _external=True)
    return google.authorize(callback=callback)
 
 
@page.route(REDIRECT_URI)
@google.authorized_handler
def authorized(resp):
    access_token = resp['access_token']
    session['access_token'] = access_token, ''
    return redirect(url_for('index_g'))
